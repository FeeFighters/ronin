...
describe "Transaction" do
  ...
  describe 'capture' do
    describe 'success' do
      before do
        @authorized_transaction = gateway.processor(processor_token).authorize(@payment_method_token, 100.0)
      end
    ...
    describe 'failures' do
      it 'should return processor.transaction - invalid with declined auth' do
        auth = gateway.processor(processor_token).authorize(@payment_method_token, 100.02)  # declined auth
      ...
      it 'should return processor.transaction - declined' do
        auth = gateway.processor(processor_token).authorize(@payment_method_token, 100.00)
  ...
  describe 'reverse' do
    describe 'on capture' do
      before do
        @purchase = gateway.processor(processor_token).purchase(@payment_method_token, 100.0)
      end
    ...
    describe 'on authorize' do
      before do
        @authorize = gateway.processor(processor_token).authorize(@payment_method_token, 100.0)
      end
    ...
    describe 'failures' do
      it 'should return input.amount - invalid' do
        purchase = gateway.processor(processor_token).purchase(@payment_method_token, 100.00)
